  ...
  Future<void> _toggleUserStatus() async {
    ...
    try {
      ...
      if (response['success'] ==
        ...
        setState(() {
          _currentUser =
              _currentUser.copyWith(status: newStatus);
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Usuario ${_currentUser.name} ha sido ${newStatus == 1
                  ? 'activado'
                  : 'desactivado'}.')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Error al cambiar el estado: ${response['message'] ??
                      'Error desconocido'}.')),
        );
      }
    } catch (e) {
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text('¿Estás seguro de que quieres eliminar a ${_currentUser
              .name}? Esta acción es irreversible.'),
          actions: <Widget>[
            TextButton(
        ...
        child: Column(
          ...
          children: <Widget>[
            Center(
              child: Column(
                children: [
                  CircleAvatar(
                    radius: 60,
                    backgroundColor: Color(0xFF899DD9).withOpacity(0.7),
            ...
            ElevatedButton.icon(
              ...
              icon: Icon(_isActive(_currentUser.status) ? Icons.lock_open : Icons.lock),
              label: Text(_isActive(_currentUser.status) ? 'Desactivar Usuario' : 'Activar Usuario'),
              style: ElevatedButton.styleFrom(
                backgroundColor: _isActive(_currentUser.status) ? Colors.orange[700] : Colors.green[700],
                foregroundColor: Colors.white,
